...
def prep_model(year: int, input_directory: str):
    ...
    with open(os.path.join("test_cases", f"test_case_{year}.pkl"), "rb") as f:
...
def tourney_sim_perfect(bracket: list, data: pd.DataFrame, names: pd.DataFrame, prnt: bool = True):
    ...
    advanced_teams = []
    rounds = []
    NAMES = ["Champion", "Finals", "Final Four", "Elite Eight", "Sweet Sixteen", "Round of 32"]
    ROUND_OPTIONS = [1, 2, 4, 8, 16, 32]
...
def simulate_brackets(input_directory: str, year: int, n: int, n_mod: int):
    ...
    avg_score = 0
    max_score = 0
    total_round_scores = [0, 0, 0, 0, 0, 0]
    avg_round_scores = [0, 0, 0, 0, 0, 0]
    max_round_scores = [0, 0, 0, 0, 0, 0]
